...
export const Template = args => ({
  ...
  setup() {
    return {
      ...
      actionAppSwitcher: () => {},
      onNotificationPanel: action('notification panel expanded'),
      onUserPanel: action('user panel expanded'),
      onSwitcherPanel: action('switcher panel expanded'),
      onSideNav: action('side nav expanded'),
      loggedIn
    }
  },
...
<cv-header aria-label="Carbon header">
  ...
  <template v-slot:right-panels>
    <cv-header-panel  id="user-panel" @onPanelResize="onUserPanel">
      An empty user panel
    </cv-header-panel>
    <cv-header-panel  id="notifications-panel" @onPanelResize="onNotificationPanel">
      An empty panel
    </cv-header-panel>
    <cv-header-panel id="switcher-panel" @onPanelResize="onSwitcherPanel">
      <cv-switcher>
        <cv-switcher-item>
...
<cv-header aria-label="Carbon header">
  ...
  <template v-slot:left-panels>
    <cv-side-nav id="side-nav" @panel-resize="onSideNav" :rail="true" :fixed="useFixed" :expanded="expandedSideNav">
      <cv-side-nav-items>
...
<cv-side-nav id="side-nav" @panel-resize="onSideNav" :rail="false" :fixed="true" :expanded="true">
  <cv-side-nav-items>
    <cv-side-nav-menu title="Home">
      <template v-slot:nav-icon><home16 /></template>
      <cv-side-nav-menu-item href="javascript:void(0)" active>Lawn care</cv-side-nav-menu-item>
